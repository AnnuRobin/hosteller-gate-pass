...
class HodsListPage extends StatefulWidget {
  const HodsListPage({Key? key}) : super(key: key);

  @override
  State<HodsListPage> createState() => _HodsListPageState();
}

class _HodsListPageState extends State<HodsListPage> {
  final AdminService _adminService = AdminService();
  final DepartmentService _departmentService = DepartmentService();
  
  List<UserModel> _allHods = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      final results = await Future.wait([
        _adminService.getUsersByRole('hod'),
        _departmentService.getAllDepartments(),
      ]);
      
      if (mounted) {
        setState(() {
          _allHods = results[0] as List<UserModel>;
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            _searchQuery.isEmpty ? 'No HODs found' : 'No matching HODs',
